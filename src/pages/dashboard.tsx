...
import { getVirtualCards } from 'apiSdk/virtual-cards';
import { VirtualCardInterface } from 'interfaces/virtual-card';
import { getTransactions } from 'apiSdk/transactions';
import { TransactionInterface } from 'interfaces/transaction';

import { TransactionListPage as ShowCaseList } from 'pages/transactions';

function DashboardPage() {
  const { session } = useSession();

  const loanStat = useSWR<PaginatedInterface<LoanInterface>>(
    () => `/loans`,
    () => getLoans({ limit: 1, offset: 0 }),
  );

  const virtualCardStat = useSWR<PaginatedInterface<VirtualCardInterface>>(
    () => `/virtual-cards`,
    () => getVirtualCards({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Loans',
      result: loanStat?.data?.totalCount,
      isLoading: loanStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Virtual Cards',
      result: virtualCardStat?.data?.totalCount,
      isLoading: virtualCardStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Transactions',
