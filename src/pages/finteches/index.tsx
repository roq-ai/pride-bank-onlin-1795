...
import { getFinteches, deleteFintechById } from 'apiSdk/finteches';
import { FintechInterface } from 'interfaces/fintech';

type ColumnType = ColumnDef<FintechInterface, unknown>;

interface FintechListPageProps {
...
export function FintechListPage(props: FintechListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<FintechInterface>>(
    () => `/finteches?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: FintechInterface) => {
    if (hasAccess('fintech', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/finteches/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/finteches/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('fintech', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/finteches/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('fintech', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
