...
import { getFinteches } from 'apiSdk/finteches';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<FintechInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/finteches` : null),
    () => getFinteches({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Finteches',
      path: isTenantUser ? `/finteches/view/${data?.[0]?.id}` : '/finteches',
      entity: 'fintech',
